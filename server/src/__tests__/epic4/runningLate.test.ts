...
describe('US 3: Running Late - BookingService.markRunningLate', () => {
    ...
    it('should throw error when user does not own the booking', async () => {
        ...
        const booking = {
            ...
            end_time: new Date(now.getTime() + 55 * 60 * 1000).toISOString(),
            status: 'CONFIRMED',
            check_in_status: 'PENDING',
            rooms: { id: 'room-789', name: 'Lab 101', code: 'LB-101' },
        };
    ...
    it('should throw error when called before booking start time', async () => {
        ...
        const booking = {
            ...
            user_id: 'user-456',
            room_id: 'room-789',
            start_time: new Date(now.getTime() + 30 * 60 * 1000).toISOString(), // 30 min in future
    ...
    it('should throw error when grace period has expired', async () => {
        ...
        const booking = {
            ...
            user_id: 'user-456',
            room_id: 'room-789',
            start_time: new Date(now.getTime() - 20 * 60 * 1000).toISOString(), // 20 min ago (> 15 min grace)
